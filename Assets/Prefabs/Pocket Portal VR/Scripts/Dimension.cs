...
public class Dimension : MonoBehaviour {
	...
	void Awake() {
		...
		// return the number of Default layer ( == 0 )
		...
		if (!this.forceKeepInitialLayers) {
			...
			foreach (Transform t in childrenTransforms) {
				...
				if (t.gameObject.GetComponent<Light> ()) {
					// Make light apply only in the given layer

					Light light = t.gameObject.GetComponent<Light> ();
					light.cullingMask = defaultLayer;
					light.cullingMask |= 1 << layer;
					//Debug.Log(System.Convert.ToString(light.cullingMask,2));
				}		
			}
		}

		// There is only one main camera.
		// Debug.Log(Camera.allCameras.Length); => 1
        foreach (Camera camera in Camera.allCameras) {
            if (this.initialWorld)
            {
				// initialize the camera: set the visible layer to camera
	...
	void Update () {
         ...
         if(mainCameraNeedsSetup)
        {
            ...
            if (this.initialWorld)
            {
				// Object which include Main Camera
				// [DY] I think below two line do the same work
